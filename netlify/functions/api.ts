...
export const handler: Handler = async (event) => {
  ...
  if (event.httpMethod !== "POST") {
    return {
      statusCode: 405,
      headers,
      body: JSON.stringify({ error: "不允許的請求方法" }),
    };
  }
  ...
  try {
    ...
    if (!csvData || csvData.trim() === "") {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: "請先輸入或粘貼 CSV 數據內容" }),
      };
    }
    ...
    if (!apiKey) {
      return {
        ...
        headers,
        body: JSON.stringify({
          error: "系統未偵測到您的 API 密鑰。請確定已在 Netlify 的環境變數中設定了 GEMINI_API_KEY。",
        }),
      };
    }
...
${customInstructions ? `額外分析需求與指示：\n${customInstructions}` : ""}`;
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: error.message || "伺服器或 API 呼叫發生未知錯誤" }),
    };
  }
};
